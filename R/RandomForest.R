    ...
    tryCatch(
      {
        ...
        if (input$ModelType == "binomial") {
          ...
          boruta_imp <- function(x) {
            ...
            sortedVariable <- boruta_variable_imp %>%
              group_by(Variable) %>%
          ...
          p1 <- sp_boxplot(boruta_variable_imp,
            melted = TRUE, xvariable = "Variable", yvariable = "Importance",
          ...
          set.seed(1)
          borutaConfirmed_rf_default <- train(
            x = boruta_train_data, y = train_data_group, method = "rf",
          ...
          best_thresh <- data.frame(coords(
            roc = roc_curve, x = "best", input = "threshold",
