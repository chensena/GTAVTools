...
server.KeggErich <- function(input, output, session) {
  ...
  observeEvent(input$Run, {
    ...
    #画图
    Go_Erich$plot <-  barplot(Go_Erich$result, showCategory=20,color="pvalue",label_format = 100,
                                   font.size=input$font_size)
    output$plot <- renderPlot(
      #fuzzy c-means 聚类，需手动定义聚类个数，比方说设置 12 个簇
      
     barplot(Go_Erich$result, showCategory=20,color="pvalue",label_format = 100,
              font.size=input$font_size)
