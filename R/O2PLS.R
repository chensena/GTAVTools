    ...
    tryCatch(
      {
        ...
        print(6)
        xj <- loadings(modelfit, "Xjoint", 1:2) %>%
          abs() %>%
          rowSums()
        ...
        gene1 <- plot(modelfit,
          loading_name = "Xj", i = 1, j = 2, label = "c", use_ggplot2 = TRUE,
          alpha = xj,
        ...
        print(9)
        # 因变量筛选
        yj <- loadings(modelfit, "Yjoint", 1:2) %>%
          abs() %>%
          rowSums()
        ...
        met1 <- plot(modelfit,
          loading_name = "Yj", i = 1, j = 2, label = "c", use_ggplot2 = TRUE,
          alpha = yj,
