...
server.heatmap <- function(input, output, session) {
  ...
  observe({ # Gene Group Annotation
    ...
    if (input$auto_gaps_row == TRUE) {
      PlotData$gap_row <- as.vector(gene_counts)
    } else {
      PlotData$gap_row <- NULL
    }
  ...
  observe({ # Sample Group Annotation
    ...
    if (input$auto_gaps_col == TRUE) {
      PlotData$gap_col <- as.vector(sample_counts)
    } else {
      PlotData$gap_col <- NULL
    }
